"""context.composer: System Prompt 组装器。
"""
...
from ..schema import Message, RoleSystem
from .skill import SkillLoader, NewSkillLoader


class PromptComposer:
    """组装发给大模型的 System Prompt，包含核心纪律、AGENTS.md、技能外挂。"""

    def __init__(self, work_dir: str | os.PathLike, plan_mode: bool = False):
        self.work_dir = Path(work_dir)
        self.plan_mode = plan_mode
        self.skill_loader: SkillLoader = NewSkillLoader(self.work_dir)
...
"""
        ...
        agents_md = self.work_dir / "AGENTS.md"
        if agents_md.exists():
            try:
        ...
        return Message(role=RoleSystem, content="".join(parts))


def NewPromptComposer(work_dir: str | os.PathLike, plan_mode: bool = False) -> PromptComposer:
    return PromptComposer(work_dir, plan_mode)


__all__ = ["PromptComposer", "NewPromptComposer"]