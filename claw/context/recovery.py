"""context.recovery: 工具执行失败时根据报错特征注入救援指引。
    ...
    def analyze_and_inject(self, tool_name: str, raw_error: str) -> str:
        ...
        if tool_name == "edit_file":
            if "在文件中未找到 old_text" in raw_error or "找不到该代码片段" in raw_error:
                hint = (
                    "你提供的 old_text 与文件当前内容不一致，或者缺少必要的缩进。请先使用 `read_file` "
                    "工具重新读取该文件，获取最新、准确的内容后，再重新发起编辑。"
                )
            elif "匹配到了多处" in raw_error or "提供更多上下文" in raw_error:
                hint = (
                    "你的 old_text 不够具体，命中了多个相同代码块。请在 old_text 中增加上下相邻的几行代码，"
                    "以确保替换的唯一性。"
                )
            ...
            elif "超时" in raw_error or "deadline" in lower_error or "timeout" in lower_error:
                hint = (
...
def NewRecoveryManager() -> RecoveryManager:
    return RecoveryManager()


__all__ = ["RecoveryManager", "NewRecoveryManager"]